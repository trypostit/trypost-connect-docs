...
title: "List API keys"
api: "GET /api-keys"
description: "List your Personal Access Tokens scoped to the current account, latest first. Returns a plain array (no pagination)."
---
...
<Note>
  The list is scoped to **the calling user in the current account**. Tokens created by other account members are not returned, even though they may have access to the same account.
</Note>
...
<RequestExample>
```bash
curl -X GET https://connect.trypost.it/api/api-keys \
