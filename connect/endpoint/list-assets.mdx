...
title: "List assets"
api: "GET /assets"
description: "List Asset Library files for the current account, 25 per page."
---

Lists files in this account's asset library. Logos, avatars, and other accounts are never included. Newest first. The dashboard has no Assets page — the library is available through the API and MCP.

Owner, Admin, and Member can call this. Viewers get `403`.

To attach an item to a post, use [`POST /posts/{post}/media/from-asset`](/connect/endpoint/attach-existing-asset). MCP: [`list-assets-tool`](/connect/mcp#assets).
...
<RequestExample>
```bash
curl -X GET "https://connect.trypost.it/api/assets?search=hero&type=image" \
...
```json
{
  ...
  "links": {
    "first": "https://connect.trypost.it/api/assets?page=1",
    "last":  "https://connect.trypost.it/api/assets?page=1",
    "prev":  null,
    "next":  null
  },
  "meta": {
    ...
    "per_page": 25,
    "total": 1,
    "path": "https://connect.trypost.it/api/assets",
    "links": [
      { "url": null, "label": "&laquo; Previous", "page": null, "active": false },
      { "url": "https://connect.trypost.it/api/assets?page=1", "label": "1", "page": 1, "active": true },
