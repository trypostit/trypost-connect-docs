...
title: "List posts"
api: "GET /posts"
description: "List all posts for the current account, paginated 25 per page."
---
...
Returns posts ordered by `scheduled_at` descending (latest first), 25 per page.
  ...
  <Expandable>
    ...
    <ResponseField name="platforms" type="array">
      ...
      <Expandable>
        ...
        <ResponseField name="content_type" type="string">Format used for this platform — e.g. `linkedin_post`, `instagram_reel`. See [`GET /content-types`](/connect/endpoint/list-content-types).</ResponseField>
        <ResponseField name="custom_content" type="string">Per-platform caption override. `null` means this platform inherits the post `content`. An empty string is an explicit blank caption.</ResponseField>
        <ResponseField name="custom_media" type="array">Per-platform media override. `null` means this platform inherits the post `media`. An empty array is an explicit no-media publish. Same item shape as root `media`.</ResponseField>
        <ResponseField name="meta" type="object">
          Per-platform settings (Pinterest `board_id`/`title`/`link`, TikTok `privacy_level`, Discord `channel_id`, etc.). May be `null` or `{}`. Full key reference: [`PUT /posts/{post}` → Per-platform meta](/connect/endpoint/update-post#per-platform-meta).
        </ResponseField>
...
<RequestExample>
```bash
curl -X GET https://connect.trypost.it/api/posts \
