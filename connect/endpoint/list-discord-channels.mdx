...
<ParamField path="account" type="string" required>
  UUID of a connected Discord social account in the current account. Non-Discord accounts return `422`.
</ParamField>

## Response

<ResponseField name="channels" type="array">
  Channels the Connect bot can post in (text and announcement channels with View Channel + Send Messages). Each item:
  <Expandable>
...
<RequestExample>
```bash
curl -X GET https://connect.trypost.it/api/social-accounts/b2c3d4e5-f6a7-8b9c-0d1e-2f3a4b5c6d7e/channels \
