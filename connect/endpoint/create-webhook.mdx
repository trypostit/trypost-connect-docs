...
Private or local endpoints are rejected with `422` and an `errors.endpoint` array. Creating does **not** ping the URL — use [send test](/connect/endpoint/send-webhook-test). The webhook is always created `enabled`. There is no `status` field on this request.
...
Returns `201` with the created webhook, including `signing_secret`. Store the secret — you can also read it later with [`GET /webhooks/{webhook}`](/connect/endpoint/get-webhook).

Same shape as [`GET /webhooks/{webhook}`](/connect/endpoint/get-webhook).
