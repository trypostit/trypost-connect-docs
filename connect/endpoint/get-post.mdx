...
title: "Get post"
api: "GET /posts/{post}"
description: "Get a specific post by ID with all its platform entries and media."
---
...
Returns the full post. Response shape is identical to one entry in [`GET /posts`](/connect/endpoint/list-posts) — see that endpoint for full field documentation (including `platforms[].meta`).

<RequestExample>
```bash
curl -X GET https://connect.trypost.it/api/posts/9f1a2b3c-4d5e-6f7a-8b9c-0d1e2f3a4b5c \
