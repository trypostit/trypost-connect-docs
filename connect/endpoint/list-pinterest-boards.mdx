...
Returns the boards Connect can publish pins to for this Pinterest account.

## Request

<ParamField path="account" type="string" required>
  UUID of a connected Pinterest social account in the current account. Non-Pinterest accounts return `422`.
</ParamField>
...
<ResponseField name="truncated" type="boolean">
  `true` when Pinterest returned more boards than Connect fetched in this call. Prefer picking from the returned list, or reconnect/retry if the board you need is missing.
</ResponseField>
...
<RequestExample>
```bash
curl -X GET https://connect.trypost.it/api/social-accounts/b2c3d4e5-f6a7-8b9c-0d1e-2f3a4b5c6d7e/boards \
...
- Do **not** call Pinterest's public API directly for this — use this endpoint so auth and scoping stay with Connect.
