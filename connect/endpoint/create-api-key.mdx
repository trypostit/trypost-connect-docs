...
Requires **manage team** permission on the account (Owner / Admin). Members and Viewers get `403`.
...
<ResponseField name="token" type="object">
  Same shape as one entry in [`GET /api-keys`](/connect/endpoint/list-api-keys) — `id`, `name`, `last_used_at`, `expires_at`, `created_at` (all timestamps ISO 8601).
</ResponseField>
...
<Warning>
  Copy the `plain_token` immediately after creation. Connect stores a hash of your API key, not the key itself. If you lose it, you'll need to create a new one.
</Warning>
