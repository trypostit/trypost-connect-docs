...
title: "Update post"
api: "PUT /posts/{post}"
description: "Update a post's content, schedule, or enabled platforms — and trigger publishing."
---
...
<ParamField body="content" type="string">
  The post caption/text body (max **10 000** characters at the API level; per-platform hard caps still apply via [`GET /content-types`](/connect/endpoint/list-content-types)). For Pinterest pins, this becomes the pin **description** (not `meta.title`).
</ParamField>

<ParamField body="media" type="array">
  Replace the post's media set. Each item must include a public `url` — the API downloads and hosts it (same shape as [`POST /posts`](/connect/endpoint/create-post)). Nested `meta.alt_text` (string, max **2000**) is allowed for accessibility — that is **media** meta, not `platforms[].meta`.

  Prefer dedicated endpoints when practical: [`POST /posts/{post}/media`](/connect/endpoint/upload-media) (multipart file), [`POST /posts/{post}/media/from-url`](/connect/endpoint/attach-media-from-url), or [`POST /posts/{post}/media/from-asset`](/connect/endpoint/attach-existing-asset) to reuse a library item.
</ParamField>
...
<ParamField body="platforms" type="array">
  ...
  <Expandable>
    ...
    <ParamField body="content_type" type="string">
      Override the content_type for this platform. Cross-validated against the social account's platform. Discover the live catalog with [`GET /content-types`](/connect/endpoint/list-content-types).
    ...
    <ParamField body="custom_content" type="string">
      Caption for this platform only. Omit the key to keep the stored override (or keep inheriting the root `content`). Send `null` to clear the override and inherit again. An empty string publishes this platform with no caption.
    </ParamField>
    <ParamField body="custom_media" type="array">
      Media for this platform only. Same item shape as root `media`. Omit the key to keep the stored override. Send `null` to inherit the root media again. Send an empty array to publish this platform without media.
    </ParamField>
...
`platforms[].meta` holds settings that only apply to one social account on the post. The same keys are accepted on [`POST /posts`](/connect/endpoint/create-post) and by the MCP create/update tools.
...
- **Media compatibility is always re-checked** on `scheduled` / `publishing`, with or without `platforms[]`: every enabled platform's effective `content_type` is validated against that platform's **effective** media (`custom_media` when set, otherwise the root `media` — media kind, GIF/MOV acceptance, byte cap via `size`, and video duration read from the file on upload). A failure is `422` on `platforms.{i}.content_type`, e.g. `Video exceeds the 300 MB limit for this post type (yours is 900.0 MB).` — see [Media → Enforced on the server](/knowledge-base/media#enforced-on-the-server-not-just-in-the-editor).
- **Caption length is measured per platform** against the effective caption (`custom_content` when set, otherwise root `content`). A long shared caption is fine if the overflowing network has a short override; a long override fails on `platforms.{i}.custom_content`.
...
| Key | Type | Required to publish? | Description |
|-----|------|----------------------|-------------|
| `document_title` | string (max 300) | No | Title shown on a **PDF document** post. If omitted, Connect falls back to the PDF filename, then `"Document"`. Only used when the post has a PDF attached. |
...
| `allow_comments` | boolean | No | Video + photo | When `true`, comments are allowed. Connect sends the inverted TikTok flag (`disable_comment`). |
...
| Key | Type | Required to publish? | Description |
|-----|------|----------------------|-------------|
| `board_id` | string | **Yes** | Target board. Resolve IDs with [`GET /social-accounts/{account}/boards`](/connect/endpoint/list-pinterest-boards) (or MCP `list-pinterest-boards-tool`). At publish time, if `board_id` is missing, Connect may fall back to the account's dashboard default board — API/MCP callers should still set `board_id` explicitly. |
...
| Key | Type | Required to publish? | Description |
|-----|------|----------------------|-------------|
| `channel_id` | string | **Yes** | Target text/announcement channel. Resolve IDs with [`GET /social-accounts/{account}/channels`](/connect/endpoint/list-discord-channels) (or MCP `list-discord-channels-tool`). |
