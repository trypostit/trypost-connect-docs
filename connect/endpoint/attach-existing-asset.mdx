...
Reuses a file already in the Asset Library. Nothing is uploaded. For a new file, use [`POST /posts/{post}/media`](/connect/endpoint/upload-media) or [`POST /posts/{post}/media/from-url`](/connect/endpoint/attach-media-from-url).

Owner, Admin, and Member can call this. Viewers get `403`.

Find IDs with [`GET /assets`](/connect/endpoint/list-assets). MCP: [`attach-existing-asset-tool`](/connect/mcp#assets).

## Path parameters

<ParamField path="post" type="string" required>
  Post UUID. Must belong to this account. Otherwise `404`.
</ParamField>

## Body parameters

<ParamField body="asset_id" type="string" required>
  Asset UUID from [`GET /assets`](/connect/endpoint/list-assets). Must be a UUID. Missing, other-account, logo, or avatar IDs return `422` on `asset_id`: `Asset not found.`
</ParamField>
...
The updated post, same shape as [`GET /posts/{post}`](/connect/endpoint/get-post). The library item is now in `media[]`.

<RequestExample>
```bash
curl -X POST https://connect.trypost.it/api/posts/9f1a2b3c-4d5e-6f7a-8b9c-0d1e2f3a4b5c/media/from-asset \
